...
const metaplex = new Metaplex(pg.connection).use(keypairIdentity(pg.wallet.keypair));
...
console.log('Creating semi fungible spl token with address: ' + createdSFT.sft.address);
...
console.log('Owned token amount: ' + tokenAccountInfo.amount / BigInt(mintDecimals));
...
console.log('Vault token amount: ' + tokenAccountInfo.amount / BigInt(mintDecimals));
...
txHash = await pg.program.methods
  .deposit(new anchor.BN(1 * mintDecimals))
  .accounts({
    tokenAccountOwnerPda: tokenAccountOwnerPda,
    vaultTokenAccount: tokenVault,
    senderTokenAccount: tokenAccount.address,
...
// should fail because the withdawl amount more than deposit amount
try {
  txHash = await pg.program.methods
    .withdraw(new anchor.BN(2 * mintDecimals))
    .accounts({
      tokenAccountOwnerPda: tokenAccountOwnerPda,
      vaultTokenAccount: tokenVault,
      senderTokenAccount: tokenAccount.address,
...
txHash = await pg.program.methods
  .withdraw(new anchor.BN(0.5 * mintDecimals))
  .accounts({
    tokenAccountOwnerPda: tokenAccountOwnerPda,
    vaultTokenAccount: tokenVault,
    senderTokenAccount: tokenAccount.address,
...
try {
  txHash = await pg.program.methods
    ...
    .accounts({
      tokenAccountOwnerPda: tokenAccountOwnerPda,
      vaultTokenAccount: tokenVault,
      senderTokenAccount: 'HNRj6w4HGqFeww3WwR1UwASYFgcArUnctRyxLWc1kkjm',
      mintOfTokenBeingSent: createdSFT.mintAddress,
      signer: pg.wallet.publicKey,
    })
    .signers([pg.wallet.keypair])
